...
class GameRules {
  ...
  static bool canMori({
    ...
    // 手札1枚なら場と同じ数字、2枚なら四則演算で一致
    if (hand.length == 1) return hand[0].number == fieldNumber;
    if (hand.length == 2) return _checkFourOperations(fieldNumber, hand[0].number, hand[1].number);
    return false;
  }

  static bool _checkFourOperations(int target, int a, int b) {
    return (a + b == target) || (a - b == target) || (b - a == target) || (a * b == target) ||
           (b != 0 && a % b == 0 && a ~/ b == target) || (a != 0 && b % a == 0 && b ~/ a == target);
  }
}